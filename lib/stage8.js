/* Stage 8 - surveillance and renewal - as pure functions over the case
   document. Ported from the tool: everything is DERIVED from the stored
...
/* A renewal is a fresh application in the tool's document shape: the facts
   carried over, every judgement blank, reference suffixed -R. */
export function renewalDoc(doc) {
  const old = doc.caseInfo || {};
  return blankDoc({ ref: old.ref ? old.ref + '-R' : '', activity: old.activity || '', provider: old.provider || '', providerEmail: old.providerEmail || '', hours: old.hours || '',
