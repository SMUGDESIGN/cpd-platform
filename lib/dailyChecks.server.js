...
import { sweepPublicDoor } from './signup.server';
...
export async function runDailyChecks() {
  ...
  const swept = await sweepPublicDoor();
  return { entries: entries.length, overdueInvoices: inv.length, notificationsWritten: sent, swept };
}
