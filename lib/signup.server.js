...
/* The website -> platform sign-up, hardened. What this file guarantees:
...
export async function gate(req) {
  ...
  if (!httpsOk(req)) { await audit({ outcome: 'bad_origin', req, note: 'not https' }); return withCors(NextResponse.json({ error: 'HTTPS required' }, { status: 400 }), origin); }
  if (!originAllowed(origin)) { await audit({ outcome: 'bad_origin', req }); return withCors(NextResponse.json({ error: 'This form can only be sent from the CPD Accreditation Scheme website' }, { status: 403 }), origin); }
  return null;
}

/* Tokens ---------------------------------------------------------------- */
function sign(nonce, ts, ipHash) { return createHmac('sha256', SECRET()).update(nonce + '.' + ts + '.' + ipHash).digest('base64url'); }
const hashNonce = (nonce) => createHash('sha256').update(nonce).digest('hex');

export async function issueToken(req) {
  const ipHash = ipHashOf(req);
  const { rows } = await query("SELECT COUNT(*)::int AS n FROM signup_tokens WHERE ip_hash = $1 AND issued_at > now() - interval '1 hour'", [ipHash]);
  ...
  const ts = Date.now();
  await query('INSERT INTO signup_tokens (nonce_hash, ip_hash) VALUES ($1, $2)', [hashNonce(nonce), ipHash]);
  return { token: nonce + '.' + ts + '.' + sign(nonce, ts, ipHash), expiresInMs: TOKEN_MAX_AGE_MS };
}
...
export async function consumeToken(token, req) {
  ...
  const ipHash = ipHashOf(req);
  const want = Buffer.from(sign(nonce, ts, ipHash)); const got = Buffer.from(sig);
  ...
  return withTransaction(async (tx) => {
    const { rows } = await tx('SELECT id, used_at, ip_hash FROM signup_tokens WHERE nonce_hash = $1 FOR UPDATE', [hashNonce(nonce)]);
...
export async function audit({ outcome, req, payload, orgId, tokenId, note }) {
  const p = payload ? { ...payload } : null;
  if (p && note) p._note = note;
  const { rows } = await query(
    'INSERT INTO signup_audit (outcome, ip_hash, origin, user_agent, payload, payload_hash, org_id, token_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id',
    [outcome, ipHashOf(req), (req.headers.get('origin') || '').slice(0, 200) || null, (req.headers.get('user-agent') || '').slice(0, 300) || null,
