/* Stage 1 - submission and completeness - as pure functions over the case
   document. Ported from the assessor tool (public/assessor/dashboard.html:
   completenessApplicable, stage1Status, stage1ReturnInfo, composeReturnEmail)
   so the Stage 1 page and the tool answer the same question the same way.
...
/* 'pass' | 'act' | 'sent' | '' - the same four words the tool and the caseload use. */
...
/* The caseload summary after a Stage 1 change. The tool's scoreEntry() puts
   the Stage 1 branches FIRST, so this can decide the verdict whenever Stage 1
   is open and leave the rest of the summary (points, gates, later stages) as
   the tool last computed it. */
export function summaryAfterStage1(doc, refinements, prev) {
  ...
  if (st1 === 'act' || st1 === 'sent') {
    ...
    /* Stage 1 just closed and the tool has not recomputed yet: say so honestly */
