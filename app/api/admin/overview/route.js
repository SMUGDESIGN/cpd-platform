...
export async function GET() {
  ...
  if (res) return res;
  const [orgs, users, cases, money, events, overdue] = await Promise.all([
    query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status='active')::int AS active FROM organisations`),
  ...
  return NextResponse.json({
    ...
    unseenEvents: events.rows, overdueInvoices: overdue.rows,
  });
}
