...
export default function AppLayout({ children, wide }) {
  ...
  useEffect(() => {
    ...
    if (role && role !== 'admin' && path.startsWith('/admin')) router.replace('/dashboard');
  ...
  const nav = (provider ? PROVIDER_NAV : STAFF_NAV).filter((n) => !n.admin || role === 'admin');
    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        <Link href={provider ? '/portal' : '/dashboard'} className="brand-lockup brand-lockup--bar">
          ...
          <span className="brand-words">Accreditation Scheme <small>{provider ? 'Provider portal' : 'Assessor platform'}</small></span>
        </Link>
