  ...
  return (
    <div className="mail">
      ...
      <div className="act-row">
        <button className="rfb" onClick={() => copy(mail.body, 'Body')}>Copy body</button>
        ...
        {extra}
        <button className="rfb btn-tiny" onClick={() => setMail(null)}>Close</button>
      </div>
      {copied && <p className="purpose">{copied}</p>}
      <p className="purpose">This composes text only - nothing sends from here. Review it, then send from your own email client.</p>
    </div>
  );
}
    ...
    <AppLayout>
      <CaseShell id={id} doc={doc} summary={sum} refinements={refinements} saveState={saveState} failed={failed}>
        ...
        <div className="panel">
          ...
          {!js.length && <p className="purpose">No mandatory gates failing.</p>}
          {js.map((job, ji) => {
            ...
            return (
              <div key={ji} className="fjob">
                <div className="fjob__head"><b>Job {ji + 1}</b> · <span className={job.cause ? '' : 'muted'}>{job.cause || 'Cause not set - name what clears it on the notice, and gates that share it become one job'}</span> · {job.rows.length} gate{job.rows.length === 1 ? '' : 's'}
                  ...
                  <button className="rfb btn-tiny" onClick={() => setNotice(job.rows[0].id)}>notice for this job</button></div>
                {job.rows.map((r) => <div key={r.id} className="fitem"><span className="muted">{r.id}</span> {r.text}{r.finding ? <em> — {r.finding}</em> : null}{!(r.notice && r.notice.contactedAt) && <span className="bad"> — no notice</span>} <button className="rfb btn-tiny" onClick={() => setNotice(r.id)}>notice</button></div>)}
              </div>
            );
          })}
          {js.length > 0 && !mail && <p><button className="rfb" onClick={() => { const m = composeProviderEmail(doc, refinements); setMail(m); setMsg(m.missingNotice ? m.missingNotice + ' of ' + m.nj + ' item(s) have no deadline logged - open that job\'s notice first if you want a firm date carried through.' : ''); }}>Compose provider notice email — all outstanding items</button></p>}
        ...
        <div className={'panel deferral ' + (!di ? (rows.length ? 'ready' : 'none') : (di.expired ? 'expired' : (di.rechecked ? (rows.length ? 'refuse' : 'done') : 'open')))}>
          ...
          {!di && !rows.length && <p className="purpose">No deferral on this entry. The action appears here when a mandatory gate is failing after moderation.</p>}
          {!di && rows.length > 0 && (
            <>
              ...
              <div className="act-row"><label>Issued by</label><input  value={who} onChange={(e) => update((d) => { d.lead = d.lead || {}; d.lead.name = e.target.value; })} style={{ maxWidth: 260 }} /><button className="rfa" disabled={!!lock} onClick={() => { if (confirm('Issue the single deferral for ' + rows.map((r) => r.id).join(', ') + '?\n\nFix window: ' + DEFERRAL_DAYS + ' days. There is no second deferral - after this window the outcome is refusal unless the window is extended with a written reason.')) act((d) => issueDeferral(d, refinements, who)); }}>Issue the single deferral</button></div>
            </>
          )}
          {di && (
            <>
              ...
              {(di.rec.extensions || []).length > 0 && <p className="purpose">Extended: {di.rec.extensions.map((x) => x.from + ' to ' + x.to + ' - ' + x.reason + ' (' + (x.by || '?') + ', ' + (x.at || '').slice(0, 10) + ')').join('; ')}</p>}
              ...
              <div className="act-row">
                {!di.rec.resubmittedAt && !di.expired && <button className="rfa" onClick={() => { if (confirm('Log the provider\'s resubmission as received today?')) act((d) => markResubmitted(d)); }}>Resubmission received</button>}
                {!di.rec.resubmittedAt && !ext && <button className="rfb" onClick={() => setExt({ days: '14', reason: '' })}>Extend the window (reason required)</button>}
                {di.rec.resubmittedAt && !di.rec.recheckedAt && <button className="rfa" onClick={() => { if (confirm('Log the re-check as complete?\n\n' + (rows.length ? rows.length + ' gate(s) still failing - the outcome becomes refusal (no second deferral).' : 'All gates are now met; the outcome goes to points.'))) act((d) => markRechecked(d, refinements)); }}>Re-check complete</button>}
              </div>
              {ext && <div className="act-row"><label>Extend by</label><input  type="number" min="1" value={ext.days} onChange={(e) => setExt({ ...ext, days: e.target.value })} style={{ width: 80 }} /> days <input  placeholder="Reason (required - recorded on the entry)" value={ext.reason} onChange={(e) => setExt({ ...ext, reason: e.target.value })} /><button className="rfa" onClick={() => { const r = act((d) => extendDeferral(d, parseInt(ext.days, 10), ext.reason, who)); if (r) setExt(null); }}>Extend</button><button className="rfb btn-tiny" onClick={() => setExt(null)}>Cancel</button></div>}
            </>
          )}
        </div>
        ...
        <div className="grid2">
          <div className="panel">
            ...
            {!ov.partial.length && <p className="purpose">Nothing partially met — scored indicators are either full marks or a clean fail.</p>}
          ...
          <div className="panel">
            ...
            {!ov.na.length && <p className="purpose">Nothing excluded — every applicable indicator is in scope.</p>}
        ...
        <div className="panel">
          ...
          <div className="act-row">
            <input  placeholder="The named action - what the provider must do" value={cond.text} onChange={(e) => setCond({ ...cond, text: e.target.value })} style={{ flex: 2 }} />
            <input  list="cond-refs" placeholder={'Indicators, e.g. ' + (partialIds[0] || '1.5.2')} value={cond.refs} onChange={(e) => setCond({ ...cond, refs: e.target.value })} style={{ width: 150, flex: 'none' }} /><datalist id="cond-refs">{partialIds.map((p) => <option key={p} value={p} />)}</datalist>
            <select  value={cond.verifyAt} onChange={(e) => setCond({ ...cond, verifyAt: e.target.value })} style={{ width: 200, flex: 'none' }}><option value="date">Verify by date</option><option value="sv1">Verify at Year-1 review</option><option value="sv2">Verify at Year-2 review</option></select>
            {cond.verifyAt === 'date' && <><span className="pills">{[28, 56, 90].map((n) => <button type="button" key={n} onClick={() => setCond({ ...cond, dueText: deadlineFrom(n) })}>+{n === 28 ? '4 weeks' : (n === 56 ? '8 weeks' : '90 days')}</button>)}</span><input  placeholder="Due, e.g. 21 November 2026" value={cond.dueText} onChange={(e) => setCond({ ...cond, dueText: e.target.value })} style={{ width: 210, flex: 'none' }} /></>}
            <button className="rfa" onClick={() => { const r = act((d) => addCondition(d, { ...cond, who })); if (r) setCond({ text: '', refs: '', verifyAt: 'date', dueText: '' }); }}>Add condition</button>
          </div>
          {!ci.all.length && <p className="purpose">No conditions on the register.{sum.computed === 'Accredited w/ conditions' ? ' The computed outcome is in the 70-84% band: name the conditions here before the decision can be signed.' : ''}{partialIds.length ? ' Partially met and available to draw on: ' + partialIds.join(', ') + '.' : ''}</p>}
          <ul className="plain">{ci.all.map((c, i) => {
            ...
            return <li key={c.id} className={'cond ' + cls}><b>C{i + 1}{c.refs && c.refs.length ? ' · ' + c.refs.join(', ') : ''}</b> {c.text}<div className="muted">Set {(c.setAt || '').slice(0, 10)} by {c.setBy || '?'} · from {condSourceLabel(c)} · verify at {condVerifyLabel(c)}{cls === 'overdue' ? <b className="bad"> · overdue</b> : null}{c.status !== 'open' ? ' · closed ' + (c.closedAt || '').slice(0, 10) + ' by ' + (c.closedBy || '?') + (c.note ? ' - ' + c.note : '') : ''}</div>
              <div className="act-row">{['open', 'met', 'unmet'].map((st) => <button key={st} type="button" className={'btn btn--tiny' + (c.status === st ? ' btn--primary' : '')} onClick={() => { if (st === 'open') act((d) => setConditionStatus(d, c.id, 'open', who)); else setClosing({ id: c.id, status: st, note: '' }); }}>{st === 'open' ? 'Open' : (st === 'met' ? 'Met' : 'Unmet')}</button>)}<button type="button" className="rfb btn-tiny" onClick={() => { if (confirm('Remove this condition from the register? Close it as Met or Unmet instead unless it was entered in error.')) update((d) => removeCondition(d, c.id)); }}>×</button></div>
              {closing && closing.id === c.id && <div className="act-row"><input  placeholder={closing.status === 'met' ? 'What was seen that shows the condition is met? (recorded)' : 'What was seen - why is the condition unmet? (recorded)'} value={closing.note} onChange={(e) => setClosing({ ...closing, note: e.target.value })} /><button className="rfa" onClick={() => { const r = act((d) => setConditionStatus(d, c.id, closing.status, who, closing.note)); if (r) setClosing(null); }}>Record {closing.status}</button><button className="rfb btn-tiny" onClick={() => setClosing(null)}>Cancel</button></div>}
            </li>;
          })}</ul>
          {ci.unmet.length > 0 && <div className="lock-note show">{ci.unmet.length} condition{ci.unmet.length === 1 ? '' : 's'} unmet - the D4 route: suspension until met. Record the grounds in the surveillance record and the remediation log.</div>}
        </div>

        <div className="panel">
          <h2>Provider communications — every notice issued on a Not-met gate, and their responses</h2>
          {!comms.length && <p className="purpose">No provider notices issued yet — when a mandatory gate is marked Not met in Stages 2–4 and you log the notice, the full thread appears here.</p>}
          {comms.map((c) => (
            <div key={c.id} className={'com ' + (c.resolved ? 'resolved' : 'open')}>
              ...
              <button className="rfb btn-tiny" onClick={() => setNotice(c.id)}>Open notice · log a reply</button>
            </div>
          ))}
        </div>
        ...
        <div className="panel">
          ...
          <div className="act-row"><textarea  rows={2} value={rlog} onChange={(e) => setRlog(e.target.value)} placeholder="Deferral issued (date), resubmission received, re-check results, regression sample…" /><button className="rfa" onClick={() => { if (!rlog.trim()) return; update((d) => logRemediation(d, rlog.trim())); setRlog(''); }}>+ Add entry</button></div>
          {!(doc.remediationLog || []).length && <p className="purpose">No entries logged yet — add one above as things happen. Deferral, resubmission, re-check and conditions write here automatically.</p>}
          <ul className="plain rlog">{(doc.remediationLog || []).slice().reverse().map((e) => <li key={e.id}><span className="muted">{when(e.ts)}</span> {e.text}{e.auto ? <span className="tag">auto</span> : null} {!e.auto && <button className="rfb btn-tiny" onClick={() => { if (confirm('Remove this log entry? This cannot be undone.')) update((d) => removeRemediationEntry(d, e.id)); }}>Remove</button>}</li>)}</ul>
        </div>
