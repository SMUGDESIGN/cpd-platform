...
function Mail({ mail, setMail }) {
  ...
  return (
    <div className="mail">
      ...
      <div className="act-row"><button className="btn" onClick={() => copy(mail.body)}>Copy body</button><a className="btn" href={'mailto:' + encodeURIComponent(mail.to) + '?subject=' + encodeURIComponent(mail.subject) + '&body=' + encodeURIComponent(mail.body)}>Open in email client</a><button className="btn btn--tiny" onClick={() => setMail(null)}>Close</button></div>
    ...
    <AppLayout>
      <CaseShell id={id} doc={doc} summary={sum} saveState={saveState} failed={failed}>
        ...
        <div className="panel">
          ...
          <p className="muted">Applicable: {sum.app} indicators. Gates: {sum.gM} met, {sum.gF} failing. Scored points: {sum.pct}%.</p>
        </div>

        {approved && (
          <div className={'sf-approve' + (sum.drift ? ' drift' : (o.accDate ? ' done' : ''))}>
            {sum.drift ? <span>Signed decision no longer matches the matrix — resolve it above before anything is issued or dated.</span>
              : (!o.accDate ? <button className="btn btn--primary" onClick={() => act((d) => approveMaterial(d, sum))}>✓ Approve this material — confirm accreditation &amp; set the dates</button>
                : <><span>✓ Accredited {o.accDate} · renews {o.expDate} · {o.accRef}{o.approvedBy ? ' · issued by ' + o.approvedBy : ''}</span><button className="btn btn--tiny" onClick={() => { if (confirm('Clear the recorded accreditation dates for this entry?')) update((d) => clearApproval(d)); }}>Clear dates</button></>)}
          </div>
        )}

        <div className={'panel deliverables' + (approved && !sum.drift ? '' : ' locked')}>
          <h2>On approval — what the provider receives</h2>
          {!(approved && !sum.drift) && <p className="muted">These deliverables unlock when the decision is signed as Accredited (or Accredited with conditions) above. Nothing here is issued from the live arithmetic, and nothing for an in-progress, held or refused submission.</p>}
          {approved && !sum.drift && (
            <>
              <div className="grid2">
                <div className="field"><label>Accreditation no.</label><input className="input" value={(doc.outcome || {}).accRef || ''} placeholder={o.accRef} onChange={(e) => update((d) => { d.outcome = d.outcome || {}; d.outcome.accRef = e.target.value; })} /></div>
                <div className="field"><label>Accredited (date)</label><input className="input" value={(doc.outcome || {}).accDate || ''} placeholder="e.g. 18 September 2026" onChange={(e) => update((d) => { d.outcome = d.outcome || {}; d.outcome.accDate = e.target.value; })} /></div>
                <div className="field"><label>Valid until</label><input className="input" value={(doc.outcome || {}).expDate || ''} placeholder="e.g. 17 September 2029" onChange={(e) => update((d) => { d.outcome = d.outcome || {}; d.outcome.expDate = e.target.value; })} /></div>
              </div>
              <div className="cards">
                <div className="case-card"><h3>1 · Unit specification &amp; certificate</h3><p className="muted">The formal accreditation document - the provider's record of exactly what was accredited (activity, hours, mode, dates, scope and the A5 boundary), with the certificate front page.</p><button className="btn" onClick={() => setSpec(true)}>Preview / print unit specification →</button></div>
                <div className="case-card"><h3>2 · Register listing</h3><p className="muted">On the platform the register comes from this signed, dated decision automatically - the public verify lookup already answers for {reg.accRef}. The row below is for the static website's register file until it is pointed at the platform.</p>
                  ...
                  <div className="act-row"><button className="btn btn--tiny" onClick={() => copy(reg.rowText, 'Register row')}>Copy register row (for register-db.js)</button><button className="btn btn--tiny" onClick={() => copy(reg.verify, 'Verify text')}>Copy verify text</button></div></div>
                <div className="case-card"><h3>3 · Accreditation badge &amp; verification</h3><div className="badge-mark"><div className="bm-cpd">CPD</div><div className="bm-acc">ACCREDITED</div><div className="bm-hours">{reg.hours ? reg.hours + ' CPD hours' : 'CPD accredited'}</div><div className="bm-ref">{reg.accRef}</div></div><p className="muted">Verify at <code>{reg.verifyUrl}</code>. QR: a scan-tested encoder is added at deployment; the placeholder shows the exact URL.</p></div>
                <div className="case-card"><h3>4 · Congratulations email</h3><p className="muted">Drafts the approval email - reference, hours, dates, what they receive, the A5 scope reminder and mark rules{ci.all.length ? ', and the conditions' : ''}.</p><button className="btn" onClick={() => setMail(composeCongratsEmail(doc))}>Compose congratulations email →</button></div>
              </div>
        ...
        <div className="panel">
          <div className="page-head"><h2>What the provider sees</h2><button className="btn btn--tiny" onClick={previewProvider}>Preview the portal view</button></div>
          <p className="muted">The provider's case page in their portal shows only what has been communicated: returned items, notices sent, the fix window, the signed outcome, conditions, review dates. Preview it before anything is said.</p>
          ...
          {pv && pv.error && <div className="alert alert--error">Could not load the preview ({pv.error}).</div>}
        </div>
        ...
        {spec && (
          <div className="overlay" onClick={(e) => { if (e.target === e.currentTarget) setSpec(false); }}>
            <div className="unitspec">
              <div className="act-row no-print"><button className="btn btn--primary" onClick={() => window.print()}>Print</button><button className="btn btn--tiny" onClick={() => setSpec(false)}>Close</button></div>
